...
Purpose:
Registers the first Tenant Admin for a newly created tenant using a one-time registration code generated by the Super Admin. Upon successful registration, ownership of the tenant is assigned to the new Tenant Admin and the registration code is permanently invalidated.
...
Request Body:
registration_code, name, email, username, password
...
Purpose:
Validates a tenant registration code before allowing registration. Returns the associated tenant information if the code is valid and has not expired.

Endpoint:
POST /cloud/auth/validate-registration-code
